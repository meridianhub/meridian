...
// TestAutoGeneratedSlugReservedWordCollision tests that auto-generated slugs
// from display names that produce reserved words are properly rejected by validation.
// For example, display name "Test" produces slug "test" which is a reserved word.
func TestAutoGeneratedSlugReservedWordCollision(t *testing.T) {
	tests := []struct {
		name        string
		displayName string
		expectSlug  string
		...
		{
			name:        "Test produces reserved slug",
			displayName: "Test",
			expectSlug:  "test",
			expectError: true,
		},
		{
			name:        "API produces reserved slug",
			displayName: "API",
			expectSlug:  "api",
			expectError: true,
		},
		{
			name:        "WWW produces reserved slug",
			displayName: "WWW",
			expectSlug:  "www",
			expectError: true,
		},
		{
			name:        "Dev produces reserved slug",
			displayName: "Dev",
			expectSlug:  "dev",
			expectError: true,
		},
		{
			name:        "Admin produces reserved slug",
			displayName: "Admin",
			expectSlug:  "admin",
			expectError: true,
		},
		{
			name:        "Test Corp is not reserved",
			displayName: "Test Corp",
			expectSlug:  "test-corp",
			expectError: false,
		},
		{
			name:        "API Services is not reserved",
			displayName: "API Services",
			expectSlug:  "api-services",
			expectError: false,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if slug != tt.expectSlug {
				t.Errorf("generateSlugFromName(%q) = %q, want %q", tt.displayName, slug, tt.expectSlug)
			}

			err := domain.ValidateSlug(slug)
			if tt.expectError && err == nil {
				t.Errorf("expected validation error for slug %q (from display name %q), but got nil", slug, tt.displayName)
			}
			if !tt.expectError && err != nil {
				t.Errorf("expected no validation error for slug %q (from display name %q), but got: %v", slug, tt.displayName, err)
			}
		})
	}
}
