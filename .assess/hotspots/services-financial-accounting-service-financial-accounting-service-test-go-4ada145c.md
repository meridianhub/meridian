# Hotspot: `services/financial-accounting/service/financial_accounting_service_test.go`

_First flagged: 2026-06-04. Last seen: 2026-06-08. Status: persistent._

## Current metrics

| Metric | Value |
|--------|-------|
| LOC | 1943 |
| Cyclomatic complexity (file max) | 116.0 |
| Commits in churn window | 11 |
| Has test file | yes |

## History across runs

| Run date | LOC | CCN | Commits | Status |
|----------|-----|-----|---------|--------|
| 2026-06-08 | 1943 | 116.0 | 11 | persistent |

## Briefing for editing this file

Use this briefing when about to modify `services/financial-accounting/service/financial_accounting_service_test.go`:

Hotspot (persistent). 1943 LOC, max cyclomatic complexity 116.0, 11 commits in churn window. (Briefing refined by LLM via assess_finalize - see Suggested actions below.)

## Suggested actions

This file is flagged but outside this run's Top 3. See the report's Top 3 Actions, or run a focused /assess pass for file-specific guidance.
