...
**When to use this runbook**: Saga transaction failures detected in CurrentAccount service orchestration with PositionKeeping and FinancialAccounting services.

> **⚠️ Note**: This runbook is specific to the saga orchestration pattern implemented in CurrentAccount→PositionKeeping→FinancialAccounting transaction flow (PR #86).
...
4. **Notify customer support** if user was impacted
...
3. **Recovery depends on findings** - follow appropriate scenario above
