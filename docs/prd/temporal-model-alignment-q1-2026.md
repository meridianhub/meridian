...
The current `measurement` table and domain model diverge from ADR-0017's temporal design:
...
| Task | Description | Effort |
|------|-------------|--------|
| 2.1 | Add `Period` type to `domain/` package with validation | S |
| 2.2 | Add `Overlaps()`, `Contains()`, `IsInstant()` methods | S |
