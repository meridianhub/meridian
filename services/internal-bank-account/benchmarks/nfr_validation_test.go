...
// for the Internal Bank Account service.
//
...
//
// Run with: go test -v ./services/internal-bank-account/benchmarks/
...
func createNFRBenchAccount(t *testing.T, tc *testContainer, codePrefix string) domain.InternalBankAccount {
	t.Helper()
...
func TestNFR_AccountLookupLatency(t *testing.T) {
	...
	// Warm up
	for i := 0; i < 10; i++ {
		_, err := tc.service.RetrieveInternalBankAccount(ctx, &pb.RetrieveInternalBankAccountRequest{
			AccountId: account.AccountCode(),
		})
		require.NoError(t, err)
	}
...
func TestNFR_ListAccountsLatency(t *testing.T) {
	...
	// Warm up
	for i := 0; i < 5; i++ {
		_, err := tc.service.ListInternalBankAccounts(ctx, &pb.ListInternalBankAccountsRequest{})
		require.NoError(t, err)
	}
...
func TestNFR_VersionInfo(t *testing.T) {
	t.Logf("=== Internal Bank Account NFR Validation ===")
	...
	t.Logf("")
	t.Logf("Run full benchmarks with: go test -v ./services/internal-bank-account/benchmarks/")
}
