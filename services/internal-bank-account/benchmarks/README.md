# Internal Bank Account Service - Performance Benchmarks
...
go test -bench=. -benchmem -benchtime=5s -timeout=30m \
  ./services/internal-bank-account/benchmarks/
```
...
go test -bench=. -benchmem -count=5 \
  ./services/internal-bank-account/benchmarks/ > baseline.txt

# After changes, compare
go test -bench=. -benchmem -count=5 \
  ./services/internal-bank-account/benchmarks/ > current.txt
