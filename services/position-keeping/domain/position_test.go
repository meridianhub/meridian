...
func TestNewPosition(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			referenceID:    uuid.New(),
			createdBy:      "system",
			wantErr:        domain.ErrInvalidDimension,
		},
		{
			name:           "empty dimension allowed (uses database default)",
			accountID:      "ACC-001",
			instrumentCode: "GBP",
			bucketKey:      "default",
			amount:         decimal.NewFromFloat(100.00),
		...
		{
			name:           "valid compute dimension",
			accountID:      "ACC-004",
			instrumentCode: "GPU_HOUR",
			bucketKey:      "cluster-a",
			amount:         decimal.NewFromFloat(24.5),
			dimension:      "Compute",
			attributes:     map[string]string{"gpu_type": "A100"},
			referenceID:    uuid.New(),
			createdBy:      "ai-scheduler",
			wantErr:        nil,
		},
		{
			name:           "valid carbon dimension",
			accountID:      "ACC-005",
			instrumentCode: "CO2_TONNE",
			bucketKey:      "project-123",
			amount:         decimal.NewFromFloat(10.0),
			dimension:      "Carbon",
			attributes:     nil,
			referenceID:    uuid.New(),
			createdBy:      "carbon-tracker",
			wantErr:        nil,
		},
	}
