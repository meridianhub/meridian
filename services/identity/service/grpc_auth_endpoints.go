...
// Password reset is intentionally not exposed as a gRPC RPC. An earlier
// implementation returned the plaintext reset token in the RPC response and
// reused long-lived invitation tokens, which enabled account takeover by email.
// The sanctioned flow lives in the api-gateway HTTP handlers
// (POST /api/v1/forgot-password and POST /api/v1/reset-password): it issues a
// short-lived, rate-limited PasswordResetToken, delivers it out-of-band via
// email, and never returns token material to the caller.
