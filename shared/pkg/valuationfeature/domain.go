...
// LifecycleStatus represents the lifecycle state of a valuation feature
type LifecycleStatus string

// Lifecycle status constants following ADR-012
const (
	LifecycleStatusInitiated  LifecycleStatus = "INITIATED"
...
func NewValuationFeature(
	...
	return &ValuationFeature{
		...
		LifecycleStatus:        LifecycleStatusInitiated,
		ValidFrom:              now,
		ValidTo:                maxTime,
		CreatedAt:              now,
		CreatedBy:              createdBy,
...
func (vf *ValuationFeature) Activate(updatedBy string) error {
	if vf.LifecycleStatus == LifecycleStatusActive {
		return nil // Idempotent
	}

	if vf.LifecycleStatus != LifecycleStatusInitiated {
		return ErrInvalidLifecycleTransition
	}

	vf.LifecycleStatus = LifecycleStatusActive
...
func (vf *ValuationFeature) Terminate(updatedBy string) error {
	if vf.LifecycleStatus == LifecycleStatusTerminated {
		return nil // Idempotent
	}
	...
	now := time.Now()
	vf.LifecycleStatus = LifecycleStatusTerminated
	vf.ValidTo = now
	vf.UpdatedAt = now
	vf.UpdatedBy = updatedBy
	return nil
...
func (vf *ValuationFeature) IsActive() bool {
	return vf.LifecycleStatus == LifecycleStatusActive
}
...
func (vf *ValuationFeature) IsTerminal() bool {
	return vf.LifecycleStatus == LifecycleStatusTerminated
}
